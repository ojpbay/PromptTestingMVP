...
export class LayoutComponent {
  ...
  lastAccuracy?: number; lastCompletedAt?: Date;

  constructor(private api: PromptTestingApiService, private promptsStore: PromptsStore) {}

  async onPromptSelected(p: Prompt) {
    this.selectedPrompt = p; this.loadingContext = true;
    try {
  ...
  onContextChanged(v: string) { this.contextText = v; }

  onTestCompleted(e: { accuracy?: number; finishedAt: Date }) {
    this.lastAccuracy = e.accuracy; this.lastCompletedAt = e.finishedAt;
  }
